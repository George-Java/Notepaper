...
exports.NoteStore = exports.AppError = void 0;
...
class NoteStore {
  ...
  notePathInCategory(fileName, category) {
  ...
  scanDirForNotes(dir, category, notes) {
  ...
  importMarkdownFile(filePath, category) {
  ...
  exportMarkdownFile(id, exportPath) {
  ...
  moveNoteToCategory(id, newCategory) {
    this.ensureStorage();
    const metadataFile = this.loadMetadata();
    const note = metadataFile.notes.find((n) => n.id === id);
    if (!note) throw AppError.noteNotFound(id);
  ...
  copyBackgroundImage(sourcePath) {
    ...
    const dest = path.join(dir, `bg-${(0, node_crypto_1.randomUUID)()}${ext}`);
    fs.copyFileSync(source, dest);
    return dest;
  }
}
exports.NoteStore = NoteStore;
function noteStoreSendToTrash(filePath) {
  try {
...
function importedMarkdownTitle(filePath, content) {
  const firstLine =
    content
      .split("\n")[0]
      ?.replace(/^\uFEFF/, "")
